...
class ComputeRappelInvoice(models.TransientModel):
    ...
    def action_invoice(self):
        ...
        tax_compute_rappel_obj = self.env["tax.rappel.calculated"]
        ...
        for rappel in compute_rappel_obj.browse(self.env.context["active_ids"]):
            ...
            if rappel.invoice_id:
                ...
                if not account_id:
                    account_id = rappel_product.categ_id.property_account_income_categ
                ...
                lineas_rappel = tax_compute_rappel_obj.search([('rappel_id','=',rappel.id)])
                for l in lineas_rappel:
                  line_tax = tax_obj.search([('base_code_id','=',l.tax_id.id),('sequence','=',0)])
                  if line_tax: 
                    tax_ids = [(4,line_tax[0].id)]
